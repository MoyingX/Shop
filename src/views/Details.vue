<template>
    ...
    <main>
      <div class="content">
        <div class="l">
          ...
          <ul>
            <li v-for="item,index in productInfo.imgAltas" :key="item.src" 
        ...
        <div class="r">
          ...
          <div class="down">
            <section v-for="item,index in productInfo.parameterJson" :key="index">
              ...
              <ul>
                <li :class="val.currentActivate?'active':''" v-for="val in item.parameters" :key="val.id" @click="toGoodsPage(val.id)">
                  {{val.title}}
                </li>
              </ul>
            </section>
      ...
      <aside>
        <h3>你还可以兑换</h3>
        <ul>
          <li v-for="item in themYouCanBuy" :key="item.id" @click="toGoodsPage(item.id)">
            <div class="l">
    ...
    <div v-show="!isShowIssue" v-html="productInfo.description">
        <!-- 商品详情的内容盒子 -->
...
export default {
    ...
    methods:{
      ...
      async getDetailsData(goodId){
        // 获取到id后，发起请求
        let res = await GoodsDetailsAPI(goodId);
        if(!res)return;
        console.log(res);
        ...
        // 对productInfo.description进行修改成,图片带前缀的格式
